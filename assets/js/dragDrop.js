...
card.forEach((item) => {
    item.addEventListener("dragover", (e) => {
        ...
        const applyAfter = getNewPosition(item, e.clientY); //e.clientY significa que a função getNewPosition está se baseando no movimento do mouse
...
function getNewPosition(cardElement, posY) {
    const cards = cardElement.querySelectorAll(".card-item:not(.dragging)");
    let result;
